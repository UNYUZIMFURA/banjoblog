  ...
  return (
    <div className="fixed hidden xl:flex flex-col justify-between py-8 px-4 w-[20rem] lg:w-[22rem] rounded-lg h-[50rem] top-10 bg-white">
      <div className="flex flex-col gap-8">
        <h1 className="text-2xl font-bold ml-4 text-primary">BANJO BLOG</h1>
        <div className="flex flex-col gap-5">
          {links.map((link, index) => (
            <div
              key={index}
              className="hover:bg-primary hover:text-white transition cursor-pointer flex items-center gap-5 p-4 rounded-md"
            >
              {link.icon}
